...
private fun MainScreen(
    ...
    LaunchedEffect(sourcesState) {
        if (sourcesState is ImportState.Template) importUrl = null
    }
...
private fun ColumnScope.SourcesTab(
    ...
    // A detected XYZ template skips the import dialog and opens the editor with the URL
    // already filled in — the user just needs to give it a name.
    LaunchedEffect(state) {
        if (state is ImportState.Template) {
            editing = TileLayer(source = "", urlTemplate = (state as ImportState.Template).url)
            viewModel.reset()
        }
    }
...
private fun ColumnScope.DmdTab(layers: List<TileLayer>, viewModel: DmdViewModel = viewModel()) {
    ...
    val fullSync by viewModel.fullSync.collectAsStateWithLifecycle()
    ...
    Column(
        ...
        if (current == null) {
            ...
            DmdSignedIn(
                ...
                onFullSync = viewModel::setFullSync,
...
private fun DmdSignedIn(
    ...
    LabelledSwitch(R.string.dmd_full_sync, fullSync, onFullSync)
